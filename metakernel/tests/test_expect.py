...
class ExpectTestCase (unittest.TestCase):
    ...
    def test_before_after(self):
        ...
        '''
        p = pexpect.spawn('%s -Wi _list100.py' % self.PYTHONBIN, env=os.environ)
    ...
    def test_greed(self):
        p = pexpect.spawn(self.PYTHONBIN + ' _list100.py')
        self._greed(p.expect)

        p = pexpect.spawn(self.PYTHONBIN + ' _list100.py', maxread=1)
    ...
    def test_signal_handling(self):
        ...
        p1 = pexpect.spawn('bash -c "echo READY; sleep 2; echo END"')
        p1.expect('READY')
        signal.alarm(1)
        p1.expect('END')
