...
class TestREPLWrapperInit:
    def test_str_command_spawns_child(self) -> None:
        ...
        with (
            patch("metakernel.pexpect.spawnu", return_value=mock_child) as mock_spawn,
            patch.object(REPLWrapper, "_expect_prompt"),
    ...
    def test_custom_encoding_passed_to_spawnu(self) -> None:
        ...
        with (
            ...
            wrapper = REPLWrapper("gnuplot", r"gnuplot>", None, encoding="cp1252")
        mock_spawn.assert_called_once_with(
            "gnuplot",
            args=None,
            echo=False,
    ...
    def test_spawn_object_used_directly(self) -> None:
        ...
        mock_child = _make_child(echo=False)
        with patch.object(REPLWrapper, "_expect_prompt"), patch("atexit.register"):
    ...
    def test_prompt_change_cmd_not_none_calls_set_prompt(self) -> None:
        ...
        with (
            patch.object(REPLWrapper, "_expect_prompt"),
            patch.object(REPLWrapper, "set_prompt") as mock_set,
            patch("atexit.register"),
        ):
    ...
    def test_extra_init_cmd_runs_command(self) -> None:
        ...
        with (
            patch.object(REPLWrapper, "_expect_prompt"),
            patch.object(REPLWrapper, "run_command") as mock_run,
            patch("atexit.register"),
        ):
            REPLWrapper(mock_child, r"[$#]", None, extra_init_cmd="export TERM=dumb")
...
class TestRunCommand:
    ...
    def test_trailing_newline_appends_empty_line(self) -> None:
        ...
        with (
            patch.object(wrapper, "_expect_prompt", return_value=0),
            patch.object(wrapper, "sendline") as mock_send,
        ):
    ...
    def test_multiline_no_prompt_emit_cmd_calls_intermediate_expect(self) -> None:
        ...
        wrapper.child.before = ""
        with (
            patch.object(wrapper, "_expect_prompt", return_value=0) as mock_ep,
            patch.object(wrapper, "sendline"),
        ):
    ...
    def test_continuation_prompt_interrupts_and_raises(self) -> None:
        ...
        wrapper = _make_wrapper()
        with (
            patch.object(wrapper, "_expect_prompt", return_value=1),
            patch.object(wrapper, "sendline"),
            patch.object(wrapper, "interrupt") as mock_interrupt,
            pytest.raises(ValueError, match="Continuation prompt"),
        ):
...
class TestInterrupt:
    ...
    def test_pty_none_calls_kill_sigint(self) -> None:
        ...
        wrapper = _make_wrapper()
        with (
            patch.object(mk_pexpect, "pty", None),
            patch.object(wrapper, "_expect_prompt", return_value=0),
        ):
            wrapper.interrupt()
...
class TestSpawnPtyNone:
    ...
    def test_no_args_uses_shlex_split(self) -> None:
        ...
        with (
            patch("metakernel.pexpect.pty", None),
            patch.object(mk_pexpect, "PopenSpawn", mock_popen_cls, create=True),
        ):
